#!/bin/bash
# =============================================================================
# gate-review.sh — Adversarial review enforcement Stop hook for bof.
#
...
# The hook command in .github/hooks/hooks.json uses "wsl bash ./scripts/..." to
...
#
# Options:
#   --strict    Block if no state file exists (used in EsquissePlan agent-scoped hook).
#               Default (no flag): fast-pass if no state file — ordinary sessions
#               are not interrupted.
#
# Exit codes:
...
# Canonical schema: SCHEMAS.md §1 (source of truth).
# =============================================================================
...
if [[ ! -f "$STATE_FILE" ]]; then
    if [[ "$STRICT" == "true" ]]; then
        ...
        # Default mode: no planning review in progress — fast-pass.
        printf '{"continue": true}\n'
        exit 0
    fi
fi
