...
| **bof-mcp** | The Go MCP server in `bof/bof-mcp/`. Exposes 5 MCP tools that give Crush (and VS Code via MCP) the equivalent of VS Code's `runSubagent` dispatch. The only non-markdown component of bof. Built with `cd bof-mcp && go build -o bof-mcp .`. |
